...
const CardStackContainer: React.FC<CardStackContainerProps> = ({
    ...
    const activeSlug = useMemo(() => {
        if (!windowModeState || windowModeState.openWindows.length === 0) return "meshstage";
    ...
    // Initialize with meshstage if empty on mobile startup
    useEffect(() => {
        if (isMobile && windowModeState && windowModeState.openWindows.length === 0) {
            windowModeState.bringToFront("meshstage");
        }
    ...
    useEffect(() => {
        ...
        // the slug to the END of openWindows (frontmost). We want MeshStage
        // on top whenever it's in the filter, so iterate other projects first
        // and MeshStage last.
        const slugsOrdered = filteredProjects
            .map(p => p.slug)
            .sort((a, b) => {
                if (a === "meshstage") return 1;   // a goes last
                if (b === "meshstage") return -1;  // b goes last
                return 0;
            });
    ...
    // Initialize mobile card order (meshstage first)
    useEffect(() => {
        if (isMobile && filteredProjects.length > 0 && mobileCards.length === 0) {
            const sorted = [...filteredProjects];
            const meshIdx = sorted.findIndex(p => p.slug === "meshstage");
            if (meshIdx > 0) {
                const [item] = sorted.splice(meshIdx, 1);
                sorted.unshift(item);
            }
